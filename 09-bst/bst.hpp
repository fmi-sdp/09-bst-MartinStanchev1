...
public:
	...
	//5 zad
	size_t weightNSubtrees(size_t n) const 
	{ 
		unordered_map<node*, size_t> map;
		return weightNSubtrees(root, n, map);
	}
private:
	...
	//5 zad
	static size_t weightNSubtrees(node* root, size_t n, unordered_map<node*, size_t>& map)
	{
		if (!root)
			return 0;
		size_t count = 0;
		count = weightNSubtrees(root->left, n, map) + weightNSubtrees(root->right, n, map);
		map[root] = 1 + map[root->left] + map[root->right];
		if (map[root] == n)
			return count + 1;
		return count;
	}
